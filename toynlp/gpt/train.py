...
class GPTTrainer:
    ...
    def _print_sample_predictions(
        ...
        print("Input tokens:", " | ".join([self.tokenizer.decode([int(token_id)])[1:] for token_id in input_ids[:50]]))
        print(
            "Target tokens:", " | ".join([self.tokenizer.decode([int(token_id)])[1:] for token_id in target_ids[:50]])
        )
        print(
            "Predicted tokens:", " | ".join([self.tokenizer.decode([int(token_id)])[1:] for token_id in pred_ids[:50]])
        )
        print("=" * 100)
