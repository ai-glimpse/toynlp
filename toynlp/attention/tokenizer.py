...
class AttentionTokenizer:
    def __init__(
        self,
        lang: str,
    ) -> None:
        self.model_path = ATTENTION_TOKENIZER_PATH_MAP[lang]
...
def train_tokenizer(lang: str, config: AttentionConfig) -> None:
    ...
    if not tokenizer_path.exists():
        attention_tokenizer = AttentionTokenizer(lang=lang)
        ...
        attention_tokenizer.tokenizer.train_from_iterator(lang_dataset["text"], trainer=trainer)
        attention_tokenizer.tokenizer.save(str(tokenizer_path))
...
def test_tokenizers(config: AttentionConfig) -> None:
    ...
    src_tokenizer = AttentionTokenizer(lang=config.source_lang).load()
    ...
    tgt_tokenizer = AttentionTokenizer(lang=config.target_lang).load()
