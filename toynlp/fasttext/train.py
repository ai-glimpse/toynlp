...
from toynlp.fasttext.dataset import get_split_dataloader
...
class FastTextTrainer:
    def __init__(self, config: FastTextConfig) -> None:
        self.config = config
        self.model = FastTextModel(self.config)
        self.model_path = FASTTEXT_MODEL_PATH
...
def train_model(config: FastTextConfig) -> None:
    ...
    val_dataset = load_dataset(
        path=config.dataset_path,
        name=config.dataset_name,
        split=f"train[{config.training_percentage}%:100%]",
    )
    ...
    val_dataloader = get_split_dataloader(
        val_dataset,  # type: ignore[unknown-argument]
        tokenizer=tokenizer,
        config=config,
    )
    test_dataloader = get_split_dataloader(
        test_dataset,  # type: ignore[unknown-argument]
        tokenizer=tokenizer,
        config=config,
    )

    trainer = FastTextTrainer(config=config)
    trainer.train(train_dataloader, val_dataloader, test_dataloader)


def main() -> None:
    """CLI entry point for training fasttext model using tyro configuration."""
